...
{
	(void)window;
	(void)xoffset;  // we aren't using horizontal scroll
	...
	// Use scroll wheel to adjust camera movement speed
	// Positive yoffset = scroll up, negative = scroll down
	gCameraSpeed += static_cast<float>(yoffset) * 0.5f;

	// Clamp so it doesn't go negative or too fast
	if (gCameraSpeed < 0.5f)  gCameraSpeed = 0.5f;
	if (gCameraSpeed > 15.0f) gCameraSpeed = 15.0f;
}
